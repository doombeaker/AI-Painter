...
def prepare_enviroment():
    ...
    run(
        ...
        "Couldn't installing requirments.txt",
    )
    ...
    if not is_installed("diffusers"):
        run(
            diffusers_command,
            "Installing oneflow-diffusers",
            "Couldn't install oneflow-diffusers",
        )

    of_transformers_repo = "https://github.com/Oneflow-Inc/transformers.git"
    git_clone(
        of_transformers_repo,
        repo_dir("transformers"),
        "OneFlow Transformers",
    ...
    if not is_installed("transformers"):
        run(
            transformers_command,
            "Installing oneflow-transformers",
            "Couldn't install oneflow-transformers",
        )
